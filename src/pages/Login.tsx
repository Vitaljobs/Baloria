  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#0F172A" }}>
      <div className="absolute inset-0 opacity-20">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 rounded-full blur-[100px]" style={{ background: "#4D96FF" }} />
        <div className="absolute bottom-1/4 right-1/4 w-48 h-48 rounded-full blur-[80px]" style={{ background: "#9D4EDD" }} />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md relative z-10">
        ...
        <Card className="border-0 shadow-high" style={{ background: "#1E293B" }}>
          <CardHeader className="text-center pb-2">
            <CardTitle className="text-2xl font-display font-bold" style={{ color: "#F1F5F9" }}>{t("login.welcome")}</CardTitle>
            <CardDescription style={{ color: "#94A3B8" }}>{t("login.subtitle")}</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <p className="mt-6 text-center text-sm" style={{ color: "#64748B" }}>
              {t("login.noAccount")}{" "}
              <Link to="/register" className="font-medium hover:underline" style={{ color: "#4D96FF" }}>{t("login.register")}</Link>
            </p>
          </CardContent>
        </Card>
      </motion.div>
